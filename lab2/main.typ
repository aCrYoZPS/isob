...
Исходный код функции main, являющейся точкой входа в разработанную программу представлен
в листинге @main_code.

Таким образом были изучены шифры Цезаря и Виженера, а также была разработана программная
реализация данных шифров.
...
алфавита на фиксированный ключ. Затем был реализован более сложный многоалфавитный шифр Виженера,
в котором роль ключа выполняет специальное кодовое слово, генерирующее последовательность сдвигов для каждого символа. 
Использование ключевой фразы позволило значительно повысить криптостойкость системы за 
...
#stp2024.listing[Реализация шифра Виженера на Python][
  ```
class Vigenere:
    @classmethod
  ...
  #stp2024.listing[Функция, являющаяся точкой входа в программу][
  ```
def main():
    run = True
    while run:
